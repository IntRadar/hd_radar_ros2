...
Введение
---
Это ROS 2 драйвер для 4D радара высокого разрешения компании Integrant (hd-radar).
...
* ### hd_radar_driver
    ...
    * ### hd_radar_driver.hpp
        Этот файл описывает класс, который используется для установления соединения с радаром, получения всех отправляемых им данных, их селекции и дальнейшей публикации в инфраструктуре ROS2. Также данный класс отвечает за обмен служебной информацией с радаром через сервисы ROS2.

        Во время инициализации класса создаются издатели и сервисы ROS2 для различных типов сообщений.
        
        Затем в отдельных потоках устанавливаются соединения с радаром по интерфейсам Ethernet и CAN. Для каждого из интерфейсов запускается бесконечный цикл, в котором осуществляется приём сообщений по протоколам UDP и CAN соответственно. По протоколу UDP приём данных осуществляется в трёх режимах: облако точек, "тепловая" карта, "сырые" данные. По протоколу CAN приём данных осуществляется только в режиме облака точек.  После приёма сообщений по каждому из протоколов, происходит их сборка в целостный кадр и публикация.
    * ### hd_radar_node.cpp
    ...
    * ### hd_radar_heat.hpp
        Этот файл описывает класс, который используется для сборки сообщений в режиме "тепловой" карты, передаваемых по протоколу UDP, и подготовки их к дальнейшей публикации в ROS2 формате. Поддерживаются "тепловые" карты дальность-скорость и дальность-азимут.
    * ### hd_radar_pcl.hpp
        Этот файл описывает класс, который используется для сборки сообщений в режиме облака точек, передаваемых по протоколу UDP, и подготовки их к дальнейшей публикации в ROS2 формате.
    * ### hd_radar_raw.hpp
        Этот файл описывает класс, который используется для сборки сообщений в режиме "сырых" данных, передаваемых по протоколу UDP, и подготовки их к дальнейшей публикации в ROS2 формате.
    * ### hd_radar_can.hpp
        Этот файл описывает класс, который используется для сборки сообщений в режиме облака точек, передаваемых по протоколу CAN, и подготовки их к дальнейшей публикации в ROS2 формате.
    * ### crc16_ccitt.hpp
        Этот файл содержит функции для расчёта контрольных сумм crc16, используемых драйвером радара.
    ...
    * ### hd_radar.launch.py
        ...
        ```
        При запуске файла создаются следующие узлы:
        - Узел визуализации, который запускает rviz 2 для отображения облака точек или карты дальность-скорость.
        - Узел работы со служебными командами радара, который запускает rqt в режиме работы с сервисами ros 2.
        - Основной узел для работы с радаром. Основных узлов может быть запущено несколько экземпляров в зависимости от количества радаров и параметров, указанных в конфигурационном файле.
...
* ### hd_radar_interfaces.
    ...
    * ### Heat.msg
        Этот файл содержит структуру входящего сообщения в режиме "тепловой" карты для карт дальность-скорость и дальность-азимут.
